...
from . import main
from ..models import Role, User, AnonymousUser, Permission, Post, Follow, Comment, Category
...
def index():
    ...
    # print(current_user._get_current_object().role.permissions)
    # print(current_user._get_current_object().role.name)

    isAnonymousUser = isinstance(current_user._get_current_object(),AnonymousUser)#没登陆的话值为True
    if isAnonymousUser:
        pagination = Post.query.filter_by(author_id = 0).order_by(Post.timestamp.desc()).paginate(#这个0并不是任何用户的id，所以也就没有文章
          page, per_page=current_app.config['FLASKY_POSTS_PER_PAGE'],
          error_out=False)
    elif current_user._get_current_object().role.name=="Administrator":#管理员的话列出所有文章
